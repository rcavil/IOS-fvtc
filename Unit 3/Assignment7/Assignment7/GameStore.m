...
-(void) connectionDidFinishLoading:(NSURLConnection*) connection
{
    ...
    else if ([operation isEqualToString:@"joingame"])
    {
        if ([status isEqualToString:@"ok"])
        {
            NSString *gameCard = [json objectForKey:@"game_card"];
            //NSArray *items = [gameCard componentsSeparatedByString:@","];
            _GameSquares = [gameCard componentsSeparatedByString:@","];
            
            NSLog(@"game card squares: %@",gameCard);
        }
        else
        {
            NSLog(@"game card message: %@",message);
        }
